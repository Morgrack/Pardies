...
socket.connect((info.wifi_server_ip, port))




#WIFI GLOBALS
global PRD0_I_Forward
PRD0_I_Forward = 0.0
global PRD0_I_Rotate
PRD0_I_Rotate = 0.0
global PRD0_I_RecImg
PRD0_I_RecImg = {}
global PRD0_I_RecImgParas
PRD0_I_RecImgParas = {}
global PRD0_O_RecImg
PRD0_O_RecImg = []
global PRD0_O_Ultrasonic
PRD0_O_Ultrasonic = 0.0
global outgoing
outgoing = b''
global incoming
incoming = b''
...
#BLOCKS: VARIABLES, LISTS, DICTIONARIES
def func0():
    global PRD0_I_RecImg
    PRD0_I_RecImg['fuck'] = ['Christmas-tree.jpg', 'Clouds.jpg', 'Dizzee and Wiley.jpg']




#BLOCKS: USER FUNCTIONS
        ...
        if PRD0_I_RecImgParas != {}:
            ...
            for key in the_dict:
                ...
                for val in the_dict[key]:
                    outgoing = outgoing + str(val).encode() + b",,,"
                to_pop.append(key)
        for key in to_pop:
...
if __name__ == '__main__':
    time.sleep(1)
    thrfunc0 = threading.Thread(target = func0)
    s_loop_thr = threading.Thread(target = sendLoop)
    r_loop_thr = threading.Thread(target = receiveLoop)
    thrfunc0.start()
    s_loop_thr.start()
    r_loop_thr.start()
